...
class SettingsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if ( !DB::table('pweb_settings')->where('setting_key', 'server_desc' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                'setting_key' => 'server_desc',
                'setting_value' => serialize('Private Server')
            ]);
        }
        ...
        if ( !DB::table('pweb_settings')->where('setting_key', 'currency_name' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                ...
                'setting_value' => serialize('Web Coins')
            ]);
        }
        ...
        if ( !DB::table('pweb_settings')->where('setting_key', 'encryption_type' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                ...
                'setting_value' => serialize('base64')
            ]);
        }
        ...
        if ( !DB::table('pweb_settings')->where('setting_key', 'chat_log_path' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                ...
                'setting_value' => serialize('/home/server/logs/')
            ]);
        }

        if ( !DB::table('pweb_settings')->where('setting_key', 'chat_num_rows' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                'setting_key' => 'chat_num_rows',
                'setting_value' => serialize(100)
            ]);
        }

        if ( !DB::table('pweb_settings')->where('setting_key', 'registration_coin' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                'setting_key' => 'registration_coin',
                'setting_value' => serialize(0)
            ]);
        }

        if ( !DB::table('pweb_settings')->where('setting_key', 'pwdata_path' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                'setting_key' => 'pwdata_path',
                'setting_value' => serialize('/home/server/pwdata')
            ]);
        }

        if ( !DB::table('pweb_settings')->where('setting_key', 'backup_path' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                'setting_key' => 'backup_path',
                'setting_value' => serialize('/var/backups')
            ]);
        }

        if ( !DB::table('pweb_settings')->where('setting_key', 'tema_depan' )->exists() )
        {
            DB::table('pweb_settings')->insert([
                'setting_key' => 'tema_depan',
                'setting_value' => serialize('default-theme')
            ]);
        }
    }
}
